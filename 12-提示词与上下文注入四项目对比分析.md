...
>
> **跨层联动**：09 —— 见第 9 章 4.4.4 与本章 2.2 末条。缓存断点打在什么位置不是模型调用层的自由，它由本章给出的**静态/动态分界**决定；本章只负责把分界画出来（CC 是四家里唯一把它做成显式标记的，见 4.4.2），把它翻译成 provider 的 `cache_control` 参数属第 9 章。这条责任链是单向的，反向不成立。
>
...
- **不管缓存断点怎么打到 API 上**（那是 L9，见第 9 章 4.4.4）。本章的子职责 7 只管**把哪一段放在前面、哪一段放在后面**，以及要不要给出一个显式的静态/动态分界；这个分界被翻译成 provider 的 `cache_control` 参数、以及「断点最多打几个」这件事，归第 9 章。两章在这里是一条单向的责任链：**分界由本章给，断点由那章打**。
...
`packages/coding-agent/src/core/skills.ts:284`
```ts
...
`packages/context/tmux-context/src/index.ts:135`
```ts
...
`codex-rs/core/src/client.rs:898`
```rust
...
`src/constants/prompts.ts:444-449`
```ts
